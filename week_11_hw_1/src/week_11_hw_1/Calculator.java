...
public class Calculator extends JFrame {
	...
	public Calculator() {
		...
		// 위치와 크기(x:8, y:10의 위치에 270*70의 크기)
	...
	// 만들어놓은 버튼에 액선 리스너 기능 추가
	// 액션 리스너를 상속시켜주고 actionPerformed(ActionEvent e)메소드로 이벤트 처리
	class PadActionListener implements ActionListener{
		public void actionPerformed(ActionEvent e) {
			...
			// =이 눌렸을 경우 위에 입력된 식을 계산, 계산값이 나오도록 함
				...
				// 밑의 메소드들을 이용하여 계산, 계산식 홤녀에 값을 띄워줌
	...
	public double calculate(String inputText) {
		...
		for(int i=0; i<equation.size(); i++) {
			...
			if(s.equals("+")) {
				...
				if((mode.equals("mul")||mode.equals("div"))&&!s.equals("+")&&!s.equals("-")&&!s.equals("×")&&!s.equals("÷")) {
					...
					// 예를 들어 3+5x6에서 3+30이 되었으므로 인덱스를 2만큼 되돌아감
					i -= 2;	// 결과값이 생긴 인덱스로 이동
				}
			}
		} // 곱셈 나눗셈을 먼저 계산한다.
		...
		for(String s : equation) {
			...
			// 소수점 여섯번 째 자리에서 반올림 -> 화면 표시 제한이 있기 때문
